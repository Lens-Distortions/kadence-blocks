...
const v6 = props => {
	...
	const renderThumbImages = ( image ) => {
		...
		const img = <div className={ imgContainClassName } style={ { paddingBottom: ( ( ( type !== 'grid' && type !== 'carousel' && type !== 'fluidcarousel' && type !== 'tiles' && type !== 'slider' && type !== 'thumbslider' ) && image.width && image.height ) || ( type === 'grid' && thumbnailRatio === 'inherit' && image.width && image.height ) ? ( ( image.height / image.width ) * 100 ) + '%' : undefined ) } }><img src={ image.thumbUrl || image.url } width={ image.width } height={ image.height } alt={ image.alt } data-full-image={ image.url } data-light-image={ image.lightUrl } data-id={ image.id } data-link={ image.link } data-custom-link={ image.customLink } data-custom-link-target={ image.linkTarget } data-sponsored={ image.linkSponsored } className={ image.id ? `wp-image-${ image.id }` : null } /></div>;
		...
		return (
				<ItemTag key={ image.id || image.url } className="kadence-blocks-gallery-item">
					<div className="kadence-blocks-gallery-item-inner">
						<figure className={ figClassName }>
							{ href ? <a href={ href } className="kb-gallery-item-link" target={ ( linkTo === 'custom' && image.linkTarget === '_blank' ) || ( linkTo === 'media' && lightbox === 'new_tab' ) ? '_blank' : undefined } rel={ ( relAttr ? relAttr : undefined ) } >{ imgPack }</a> : imgPack }
						</figure>
					</div>
				</ItemTag>
		);
